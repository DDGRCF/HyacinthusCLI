...
- 整理后的字段只能使用并严格按这个顺序输出：`编号`、`年级`、`科目`、`需求方性别`、`薪资`、`时间`、`地址`、`老师性别要求`、`老师学历要求`、`老师学校要求`、`老师资格要求`、`要求`、`备注`。
- 缺失字段填 `未填写`，不要省略字段。
- 不要新增 `性别`、`微信`、`联系方式`、`授课时间`、`老师要求` 等临时字段；原文写 `薪酬` 时整理到 `薪资`。
...
- 需求方、学生、家长侧性别放 `需求方性别`；老师性别筛选放 `老师性别要求`。不要再使用旧字段 `性别`，也不要把老师性别要求塞进 `要求`。
- 老师学历、学校、资格分别放 `老师学历要求`、`老师学校要求`、`老师资格要求`；只有无法结构化的补充说明才放 `要求` 或 `备注`。
